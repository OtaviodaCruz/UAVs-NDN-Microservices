...
def setRequests(graphObj, podObj):        
    total_requests_cpu = Decimal(0)
    total_requests_memory = Decimal(0)

    for i in podObj.spec.containers:
        if i.resources:
            requested_cpu = utils.parse_quantity(i.resources.requests["cpu"])  
            total_requests_cpu += requested_cpu
    
            requested_memory = utils.parse_quantity(i.resources.requests["memory"])
            total_requests_memory =+ requested_memory
        else:
            log.printWithColor(podObj.metadata.name + " don't have field named resources", type="CRITICAL")
            log.writeDataLogFile(podObj.metadata.name, " don't have field named resources",  type="CRITICAL")
            return False  
    
    graphObj.nodes[podObj.metadata.name]["cpu_stats"]["cpu_total"] = total_requests_cpu
    graphObj.nodes[podObj.metadata.name]["memory_stats"]["memory_total"] = total_requests_memory
